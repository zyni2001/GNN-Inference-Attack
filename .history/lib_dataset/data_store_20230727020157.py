...
class DataStore:
    ...
    def load_raw_data(self, dataset_name):
        ...
        if self.args['is_use_feat']:
            ...
            # # first load the dataset without any transformations and just filter out the graphs with too many nodes
            dataset = TUDataset(config.RAW_DATA_PATH + str(self.max_nodes) + '/', name=dataset_name, 
                                use_node_attr=True, use_edge_attr=False, pre_filter=MyFilter(self.max_nodes), pre_transform=pre_transform)
            # then transform the dataset to dense format
            dataset = dataset.transform(T.ToDense(self.max_nodes))
