...
class DataStore:
    ...
    def determine_data_path(self):
        ...
        # parent directory of the target model: config.TARGET_MODEL_PATH + str(self.max_nodes) + '/'
        # if the parent directory does not exist, then create it.
        if not os.path.exists(config.TARGET_MODEL_PATH + str(self.max_nodes) + '/'):
            os.makedirs(config.TARGET_MODEL_PATH + str(self.max_nodes) + '/')
        ...
        if not os.path.exists(config.SHADOW_MODEL_PATH + str(self.max_nodes) + '/'):
            os.makedirs(config.SHADOW_MODEL_PATH + str(self.max_nodes) + '/')
        ...
        # parent directory of the target model: config.ATTACK_DATA_PATH + 'subgraph_infer_2/'
        # if the parent directory does not exist, then create it.
        if not os.path.exists(config.ATTACK_DATA_PATH + 'subgraph_infer_2/'):
            os.makedirs(config.ATTACK_DATA_PATH + 'subgraph_infer_2/')
    ...
    def load_raw_data(self, dataset_name):
        ...
        if self.args['is_use_feat']:
            ...
            # # first load the dataset without any transformations
            # dataset = TUDataset(config.RAW_DATA_PATH + str(self.max_nodes) + '/', name=dataset_name)

            # # get the maximum number of nodes
            # max_nodes = self.get_max_nodes(dataset)
            
            # dataset = TUDataset(config.RAW_DATA_PATH + '/'.join((str(max_nodes), 'non_feat')) + '/', name=dataset_name,
            #     use_node_attr=True, use_edge_attr=False, transform=T.ToDense(max_nodes),
            ...
            dataset = TUDataset(config.RAW_DATA_PATH + '/'.join((str(self.max_nodes), 'non_feat')) + '/', name=dataset_name,
                         use_node_attr=True, use_edge_attr=False, pre_filter=MyFilter(self.max_nodes), transform=T.ToDense(self.max_nodes),
                          pre_transform=pre_transform)

        self.transform=T.ToDense(self.max_nodes)
        for i, data in enumerate(dataset):
            try:
                print(f"Processing graph {i}, Number of nodes: {data.num_nodes}")
                data = data if self.transform is None else self.transform(data)
            except AssertionError:
                print(f"Graph {i} caused an error. Number of nodes: {data.num_nodes}")
                break
