...
sample_node_ratios=(0.2) # 0.4 0.6 0.8)
target_models=(diff_pool) # mincut_pool mean_pool)
sample_methods=(snow_ball) # forest_fire)
max_nodes=$1
num_runs=$2
        ...
        do
          for sample_method in ${sample_methods[@]}
            do
              ...
              config=$dataset_name"_"$sample_node_ratio"_"$target_model"_"$sample_method
              mkdir -p ./temp_data_$config/log/
              mkdir -p temp_data_$config/embed_log/
              rm -rf temp_data_$config/raw_data/500/feat/DD/
              # paper
              ...
              #### genrate data every time
              # cmd="python main.py --configure $config --is_vary False --dataset $dataset_name --shadow_dataset $dataset_name --exp 'subgraph_infer' --cuda $gpu_id --num_threads 1 --is_split True --is_use_feat True --is_train_target_model True --is_use_shadow_model False --is_train_shadow_model False --is_upload False --num_runs $num_runs --target_model $target_model --shadow_model $target_model --max_nodes $max_nodes --target_ratio 0.4 --attack_train_ratio 0.3 --attack_test_ratio 0.3 --batch_size 32 --num_epochs 100 --is_gen_attack_data True --train_sample_method $sample_method --test_sample_method $sample_method --sample_node_ratio $sample_node_ratio --attack 'subgraph_infer_2' 2>&1 | tee "./temp_data_$config/log/$dataset_name.$target_model.$sample_node_ratio.$sample_method.max_nodes$max_nodes.log" &"
              #### use generated data
              cmd="python main.py --configure $config --is_vary False --dataset $dataset_name \
                  --shadow_dataset $dataset_name --exp 'subgraph_infer' --cuda $gpu_id --num_threads 1 \
                  --is_split False --is_use_feat True --is_train_target_model False --is_use_shadow_model False \
                  --is_train_shadow_model False --is_upload False --num_runs $num_runs \
                  --target_model $target_model --shadow_model $target_model --max_nodes $max_nodes \
                  --target_ratio 0.4 --attack_train_ratio 0.3 --attack_test_ratio 0.3 --batch_size 32 \
                  --num_epochs 100 --is_gen_attack_data False --train_sample_method $sample_method \
                  --test_sample_method $sample_method --sample_node_ratio $sample_node_ratio \
                  --attack 'subgraph_infer_2' --is_use_our_embedding True \
                  > ./temp_data_$config/embed_log/new_exp.log 2>&1 &"
                  # 2>&1 | tee ./temp_data_$config/log/$dataset_name.$target_model.$sample_node_ratio.$sample_method.max_nodes$max_nodes.log &"
