...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        x0 = x
        x1 = self.bn(1, self.dropout(self.conv1(x0, adj, mask).relu()))
        x2 = self.bn(2, self.dropout(self.conv2(x1, adj, mask).relu()))
        x3 = self.bn(3, self.dropout(self.conv3(x2, adj, mask).relu()))
...
def train(epoch):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        output, mc_loss, o_loss = model(data.x, data.adj, data.mask)
